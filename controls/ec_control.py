...
for i, (loss, size) in enumerate(zip(han_losses, han_sizes)):
    ax.scatter(loss, size, marker='o', color=cmap(i/len(han_losses)), edgecolor='k', s=70, alpha=0.75)
cmap = plt.get_cmap('autumn')
for i, (loss, size) in enumerate(zip(lth_losses, lth_sizes)):
...
fig.tight_layout()
fig.savefig(os.path.join(home_location, 'controls', 'oto_sweep', 'control_ec_neurons.svg'))
fig.show()

fig2, ax2 = plt.subplots(figsize=(3, 4.5), dpi=200)
for gamma, beta, loss, size in ec_sweeps:
    if gamma < 0.2:
        continue
    ax2.scatter(np.sqrt(loss), size, edgecolor='r', marker='*', s=15, facecolor='none', zorder=-1)
    ax2.annotate(f'$\gamma={{{gamma}}}, \\beta=\pi/{{{beta}}}$', (np.sqrt(loss)*1.025, size), color='k', fontsize=6)
ax2.set_xscale('log')
ax2.set_xlim([4e-4, 5.5e-4])
ax2.set_ylim([1, 1.4])
fig2.tight_layout()
fig2.savefig(os.path.join(home_location, 'controls', 'oto_sweep', 'control_ec_neurons2.svg'))
fig2.show()