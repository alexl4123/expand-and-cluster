...
# Dummy optimizer, it is going to be overwritten after restoring the checkpoint
optimizer = oto.dhspg(lr=1e-3, epsilon=0.95, variant='adam', target_group_sparsity=0.80,
                      start_pruning_steps=2000 * len(train_loader),
                      weight_decay=0.0)
checkpoint_location = pathlib.Path().resolve().joinpath('controls', 'oto_sanity_check')

cp_step, cp_logger = restore_checkpoint(checkpoint_location, model, optimizer, train_loader.iterations_per_epoch)
